...
const char *MYRISCVXTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch (Opcode) {
  case MYRISCVXISD::TailCall:
    return "MYRISCVXISD::TailCall";
  case MYRISCVXISD::Hi:
    return "MYRISCVXISD::Hi";
  case MYRISCVXISD::Lo:
    return "MYRISCVXISD::Lo";
  case MYRISCVXISD::GPRel:
    return "MYRISCVXISD::GPRel";
  case MYRISCVXISD::Ret:
    return "MYRISCVXISD::Ret";
  case MYRISCVXISD::EH_RETURN:
    return "MYRISCVXISD::EH_RETURN";
  case MYRISCVXISD::DivRem:
    return "MYRISCVXISD::DivRem";
  case MYRISCVXISD::DivRemU:
    return "MYRISCVXISD::DivRemU";
  case MYRISCVXISD::Wrapper:
    return "MYRISCVXISD::Wrapper";
  default:
    return NULL;
  }
}
...
SDValue MYRISCVXTargetLowering::LowerFormalArguments(
  ...
  for (unsigned i = 0, e = ArgLocs.size(); i != e; ++i) {
    ...
    if (IsRegLoc) {
      ...
      const TargetRegisterClass *RC = getRepRegClassFor(RegVT);

      // レジスタに配置された引数を、仮想的な変数に乗せ替えるために
      // getCopyFromRegを使用する
...
void MYRISCVXTargetLowering::writeVarArgRegs(std::vector<SDValue> &OutChains,
  ...
  // Offset of the first variable argument from tack pointer.
  int VaArgOffset;
  if (ArgRegs.size() == Idx) {
