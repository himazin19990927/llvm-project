...
static MCRegisterInfo *createMYRISCVXMCRegisterInfo(const Triple &TT) {
  ...
  InitMYRISCVXMCRegisterInfo(
      X, MYRISCVX::RA); // defined in MYRISCVXGenRegisterInfo.inc
  return X;
}
// @} MYRISCVXMC_TargetDesc_cpp_createMYRISCVXMCRegisterInfo
...
createMYRISCVXMCSubtargetInfo(const Triple &TT, StringRef CPU, StringRef FS) {
...
static MCTargetStreamer *
createMYRISCVXAsmTargetStreamer(MCStreamer &S, formatted_raw_ostream &OS,
...
extern "C" void LLVMInitializeMYRISCVXTargetMC() {
  for (Target *T : {&getTheMYRISCVX32Target(), &getTheMYRISCVX64Target()}) {
    ...
    TargetRegistry::RegisterObjectTargetStreamer(
        *T, createMYRISCVXObjectTargetStreamer);
